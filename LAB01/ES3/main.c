...
void print(LOG *tabella, int n, char *name, int option){
    ...
    if(option==1){
        ...
        if(option%2!=0)
            printf("%s %s %s %s %s %s %d\n", tabella->codice_tratta, tabella->partenza, tabella->destinazione,
                   tabella->data, tabella->ora_partenza, tabella->ora_arrivo, tabella->ritardo);
        else{
            fout=fopen(name,"aw");
            if(fout==NULL){
                printf("Errore durante la creazione del file\n");
...
void findAlgorithm(LOG *tabella,int n){
    ...
    int opt,upper_bound, lower_bound,p_opt,i,found=1;
    ...
    if(!found){
        ...
        sorting(tabella,n,o_partenza);
        lower_bound=BinSearch_lower(tabella,0,n,str);
        upper_bound=BinSearch_upper(tabella,0,n,str);
        printf("Scegliere se visualizzare il risultato a video o esportare su un file:\n1-Video\n2-File\n"
               "Comando (1 o 2): ");
        scanf("%d",&p_opt);
        linearSearch(tabella,n, lower_bound,upper_bound, p_opt);
    }
}
...
void menuParola(LOG *tabella, int n){
    ...
    while(continua){
        ...
        switch(cmd){
            case o_date:
                sorting(tabella,n,o_date);
                print(tabella,n,"SortByDate.txt",1);
                break;
            case o_codice:
                sorting(tabella,n,o_codice);
                print(tabella,n,"SortByID.txt",1);
                break;
            case o_partenza:
                sorting(tabella,n,o_partenza);
                print(tabella,n,"SortByDeparture.txt",1);
                break;
            case o_arrivo:
                sorting(tabella,n,o_arrivo);
                print(tabella,n,"SortByArrival.txt",1);
                break;
            case ricerca:
